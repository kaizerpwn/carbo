    ...
    <div className="bg-backgroundLight rounded-xl p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          >
            <Power
              className={`w-5 h-5 ${
                device.isActive ? "text-white" : "text-[#6B7280]"
              }`}
            />
          </button>
          <div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => onFavoriteToggle(device.id)}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
        <div className="text-[#6B7280]">
          Power Rating:{" "}
          <span className="text-white">{device.powerRating}W</span>
        </div>
        <div className="text-[#6B7280]">
          Standby: <span className="text-white">{device.standbyPower}W</span>
        </div>
      </div>
      ...
      {device.schedules && device.schedules.length > 0 && (
        <div className="mt-2 text-sm text-[#6B7280]">
          <div className="flex items-center gap-1">
            ...
            <span>
              {device.schedules[0].on} - {device.schedules[0].off}
            </span>
          </div>
        </div>
      )}
